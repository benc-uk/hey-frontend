...
function refresh() {
  ...
  var connAvg = 0;  var connAvgCount = 0; 
  var dnsAvg = 0;   var dnsAvgCount = 0; 
  var writeAvg = 0; var writeAvgCount = 0; 
  var waitAvg = 0;  var waitAvgCount = 0; 
  var readAvg = 0;  var readAvgCount = 0; 
  ...
  Papa.parse(`/data/${csvName}`, {
    ...
    step: function(row) {
      ...
      if(row.data[0][1] > 0) {
        connAvg += parseFloat(row.data[0][1]);
        connAvgCount++
      }
      if(row.data[0][2] > 0) {
        dnsAvg += parseFloat(row.data[0][2]);
        dnsAvgCount++
      }
      if(row.data[0][3] > 0) {
        writeAvg += parseFloat(row.data[0][3]);
        writeAvgCount++
      }
      if(row.data[0][4] > 0) {
        waitAvg += parseFloat(row.data[0][4]);
        waitAvgCount++
      }
      if(row.data[0][5] > 0) {
        readAvg += parseFloat(row.data[0][5]);
        readAvgCount++
      }
